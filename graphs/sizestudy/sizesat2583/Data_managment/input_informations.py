...
if __name__ == "__main__":
    file_path = "sizesat2583/SARA/REENTRY/input/objects.xml"
...
def main():
    # 1) On calcule le chemin vers objects.xml **relatif au script lui-même**
    base_dir = os.path.dirname(os.path.realpath(__file__))
    file_path = os.path.normpath(
        os.path.join(base_dir, "..", "SARA", "REENTRY", "input", "objects.xml")
    )

    # 2) On vérifie que ça existe
    if not os.path.isfile(file_path):
        raise FileNotFoundError(f"XML introuvable ici : {file_path}")

    # 3) On demande le seuil et on affiche
    user_input = input("Enter mass threshold (or 'all' to show all objects): ")
    if user_input.lower() == 'all':
        objects, relationships = parse_objects(file_path, show_all=True)
        print(f"Total number of objects taken into account: {len(objects)}")
