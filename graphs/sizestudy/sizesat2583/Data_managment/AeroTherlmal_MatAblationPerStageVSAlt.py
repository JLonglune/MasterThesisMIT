...
############################################
def plot_material_contributions_by_hierarchy_level(xml_path, data_path, mass_threshold=None):
    ...
    # Objets présents dans le fichier de masse
    available_object_names = set(df["Objet"].unique())

    # Regrouper les objets par étage
    level_to_objects = {}
    ...
    for i, lvl in enumerate(sorted_levels):
        ...
        for uid in uid_list:
            ...
            if name not in available_object_names:
                print(f"[INFO] '{name}' absent du fichier de données.")
                missing_objects.append(name)
                continue

            elif obj_df.empty:
                print(f"[INFO] '{name}' présent mais aucune ligne trouvée (peut-être des NaN ?)")
                missing_objects.append(name)
                continue

            elif obj_df["Mass (kg)"].isnull().all() or obj_df["Altitude (km)"].isnull().all():
                print(f"[INFO] '{name}' présent mais toutes les valeurs sont nulles.")
                missing_objects.append(name)
                continue

            if material not in material_groups:
                material_groups[material] = []
            material_groups[material].append(obj_df)


        if missing_objects:
            print(f"[INFO] Objets du niveau {lvl} ignorés (pas trouvés dans les données) : {missing_objects}")

        for material, obj_dfs in material_groups.items():
            combined_df = pd.concat(obj_dfs)
            combined_df = combined_df.sort_values("Altitude (km)")
            ...
            for obj_df in obj_dfs:
                ...
                altitudes = obj_df["Altitude (km)"].values
                masses = obj_df["Mass (kg)"].values
                interp_mass = np.interp(altitude_grid, altitudes, masses)
                total_mass += interp_mass

            ax.plot(total_mass, altitude_grid, marker='o', linestyle='-', linewidth=1.5,
                    label=material)

        ax.set_title(f"Niveau hiérarchique : {lvl}")
        ax.set_xlabel("Masse totale (kg)")
        ax.set_ylabel("Altitude (km)")
        ax.grid(True)
        ax.legend(title="Matériau", fontsize=7, loc='best')

    fig.suptitle("Contribution massique par matériau et par niveau hiérarchique", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.show()



xml_path = "sizesat2583/SARA/REENTRY/input/objects.xml"
data_path = "sizesat2583/Data_managment/MassMat_by_object_file.txt"
plot_material_contributions_by_hierarchy_level(xml_path, data_path, mass_threshold=5)
