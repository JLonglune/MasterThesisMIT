...
def transformer_dimension(valeur_str):
    ...
    nouvelle_valeur = valeur * 0.72
    return f"{nouvelle_valeur:.3f}"

# Charger le fichier XML original.
tree = ET.parse("sizesat1000/SARA/REENTRY/input/objects_fullsize.xml")
root = tree.getroot()
...
for objet in root.findall("object"):
    ...
    if masse is not None and masse.text:
        try:
            ...
            masse.text = f"{masse_num / 2.583:.3f}"
        except ValueError:
...
tree.write("sizesat1000/SARA/REENTRY/input/objects_transformed.xml", encoding="UTF-8", xml_declaration=True)
